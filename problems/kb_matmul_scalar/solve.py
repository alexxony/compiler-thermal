...
def reference(case, device):
    # A100 Colab 환경은 torch.backends.cuda.matmul.allow_tf32 기본값 True.
    # 이 문제의 seed는 "TF32 토글에 영향받지 않는 scalar 커널"이 전제라
    # reference(cuBLAS)도 TF32가 섞이면 안 됨 — 안 그러면 gate가 커널이 아니라
    # reference의 TF32 오차를 잡아버림 (max_err 3e-2, K에 비례해 누적).
