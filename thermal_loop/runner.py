...
class MailboxGlue:
    ...
    """
    def __init__(self, generator, profiler: MailboxProfiler, submit_kind: str | None = None):
        self.gen = generator
        self.profiler = profiler
        self.submit_kind = submit_kind   # Compiler_Thermal P3: "thermal" → ColabExecProfiler에 실림
    ...
    def _submit(self, code: str, problem: str) -> MailboxResult:
        ...
        if key not in self._cache:
            if self.submit_kind:
                self._cache[key] = self.profiler.submit(code, problem, kind=self.submit_kind)
            else:
...
def run_problem(
    ...
    submit_kind: Compiler_Thermal P3 — "thermal"이면 매 submit에 cmd["kind"]="thermal"이
      실려 executor._profile_thermal로 분기(ncu+power 분리 런). None(기본)=기존 동작.
    """
    if profiler is None:
    ...
    glue = MailboxGlue(gen, profiler, submit_kind=submit_kind)
